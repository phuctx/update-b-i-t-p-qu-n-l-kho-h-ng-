...
namespace QuanLyXuatNhapKho
{
    public partial class Frm_KhachHang : Form
    {
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            txtMakhachhang.Text = SinhMaTuDong("kh");
            txtTenkhachhang.Focus();
        }
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn có muốn xóa không ?", "Thông báo", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
            {
                ...
                cmd = new SqlCommand("DELETEKhachHang", conn);
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            ...
            if (Trangthai == "ADD")
            {
                if (CheckTXT())
                {
                    sMa = txtMakhachhang.Text;
                    cmd = new SqlCommand("ThemKhachHang", conn);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@makh", txtMakhachhang.Text);
                    cmd.Parameters.AddWithValue("@tennkh", txtTenkhachhang.Text);
            ...
            else if (Trangthai == "EDIT")
            {
                if (CheckTXT())
                {
                    cmd = new SqlCommand("EDITKhachHang", conn);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@makh", txtMakhachhang.Text);
                    cmd.Parameters.AddWithValue("@tennkh", txtTenkhachhang.Text);
                    cmd.Parameters.AddWithValue("@diachi", txtDiachi.Text);
        ...
        private void dgvData_SelectionChanged(object sender, EventArgs e)
        {
            if (dgvData.SelectedCells.Count > 0 && dgvData.SelectedRows.Count > 0)
            {
                ...
                txtMakhachhang.Text = dgvData.Rows[Index].Cells[0].Value.ToString();
                txtTenkhachhang.Text = dgvData.Rows[Index].Cells[1].Value.ToString();
                txtSodienthoai.Text = dgvData.Rows[Index].Cells[3].Value.ToString();
